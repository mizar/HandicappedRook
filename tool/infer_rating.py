...
def infer_rating(win, lose, draw=0): # レーティングの区間推定を行う
    ...
    def print_result(result, sigma): # 結果の出力
        ...
        print(f"{sigma}σ: R({100 * result[2]:.7f}%) {lower:+8.2f} ({lowerrate:6.2f}%) ～ {upper:+8.2f} ({upperrate:6.2f}%), Range:{upper - lower:7.2f}")
    ...
    print_result(clopper_pearson(win, match, 0.99999320465375074), 4.5)
    ...
    print_result(clopper_pearson(win, match, 0.99999996202087527), 5.5)
    ...
    print_result(clopper_pearson(win, match, 0.99999999802682460), 6.0)
